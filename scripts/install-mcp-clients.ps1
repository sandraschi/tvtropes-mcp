...
foreach ($cfg in $configs) {
    ...
    try {
        ...
        if ($json.mcpServers."tvtropes-mcp" -eq $null) {
            $json.mcpServers | Add-Member -Name "tvtropes-mcp" -Value @{ url = "http://127.0.0.1:10964/mcp" } -MemberType NoteProperty
            $json | ConvertTo-Json -Depth 10 | Set-Content $cfg.Path
            Write-Host "  Registered tvtropes-mcp in $($cfg.Name)" -ForegroundColor Green
            $updated++
        } else {
