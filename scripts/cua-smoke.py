...
BACKEND_PORT = int(cfg("backend_port", 10964))
BACKEND_URL = f"http://127.0.0.1:{BACKEND_PORT}"
PRODUCT_NAME = cfg("product_name", "TVTropes MCP")
...
WINDOW_TITLE_RE = cfg("window_title_re", "TVTropes MCP")
BRIDGE_OK_TEXT = cfg("bridge_ok_text", "API reachable")
INSTALL_DIR = cfg("install_dir", "%LOCALAPPDATA%\\TVTropes MCP")
OPERATOR_EXE = cfg("operator_exe", "tvtropes-mcp-native.exe")
PROCESS_NAMES = cfg("backend_process_names", ["tvtropes-mcp-native", "tvtropes-mcp-backend"])
NSIS_GLOB = cfg("nsis_glob", "native/target/release/bundle/nsis/TVTropes MCP_*_x64-setup.exe")
REGISTRY_FILTER = cfg("uninstall_registry_filter", "*TVTropes*")
MAX_RETRY = 10
RETRY_DELAY = 3
...
def analyze_logs():
    ...
    log_paths = [
        os.path.join(INSTALL_DIR, "tvtropes-mcp.log"),
        os.path.expandvars(r"%APPDATA%\com.sandraschi.tvtropes-mcp\logs\backend-spawn.log"),
    ]
    errors = []
    warnings = []
